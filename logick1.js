...
function isKingInCheck(player, board) {
  // Implement the logic to check if the king of the specified player is in check
  // You need to check the board state, the positions of the pieces, and the legal moves
  // This will depend on the rules of chess, and you may need to implement specific logic for each piece type.
  // For simplicity, you can start with a basic implementation for the king's movement.

  // Example: Check if the king is in check by an opponent's knight
  const opponentWknightMoves = [
  ...
  const kingPosition = findPiecePosition('K', player, board);

  for (const move of opponentKnightMoves) {
    const newRow = kingPosition.row + move.row;
    const newCol = kingPosition.col + move.col;

    if (isValidPosition(newRow, newCol) && board[newRow][newCol] === 'n') {
...
function findPiecePosition(piece, player, board) {
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      if (board[row][col] === piece && getPlayer(board[row][col]) === player) {
        return { row, col };
      }
    }
  }
...
// Function to get the player (color) of a piece
